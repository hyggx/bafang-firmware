...
### 📡 接收 / 发射
- 宽频接收（超出业余频段的接收能力）
- 多级发射功率：Low1–Low5（约 20mW–1W）、Mid（约 2W）、High（约 5W）、自定义
...
输出文件位于 `build/Fusion/`，包含 `.elf`、`.bin`、`.hex`。

**可用 Preset：** `Custom` / `Bandscope` / `Broadcast` / `Basic` / `RescueOps` / `Fusion`
