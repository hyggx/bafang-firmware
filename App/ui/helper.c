...
/* --------------------------------------------------------------------------
 * UTF-8 aware big-font string renderer
 * Handles ASCII (gFontBig, 7×14px, 2 rows) and CJK glyphs (12×12px, 2 rows).
 * Framebuffer byte convention: LSB = topmost pixel within the 8-pixel page.
 * -------------------------------------------------------------------------- */

static uint32_t s_utf8_next(const uint8_t **pp)
{
    uint8_t b = **pp;
    if (!b)           return 0;
    if (b < 0x80u)  { (*pp)++; return b; }
    if (b < 0xC0u)  { (*pp)++; return 0xFFFDu; }  /* stray continuation */
    if (b < 0xE0u) {
        if (!(*pp)[1]) { (*pp)++; return 0xFFFDu; }  /* truncated sequence */
        uint32_t cp = ((uint32_t)(b & 0x1Fu) << 6) | ((*pp)[1] & 0x3Fu);
        *pp += 2; return cp;
    }
    if (b < 0xF0u) {
        if (!(*pp)[1] || !(*pp)[2]) { (*pp)++; return 0xFFFDu; }  /* truncated */
        uint32_t cp = ((uint32_t)(b & 0x0Fu) << 12)
                    | ((uint32_t)((*pp)[1] & 0x3Fu) << 6)
                    | ((*pp)[2] & 0x3Fu);
        *pp += 3; return cp;
    }
    *pp += 4; return 0xFFFDu;  /* > U+FFFF */
}

static void s_draw_cjk(const uint8_t bmp[CJK_GLYPH_BYTES], uint8_t x, uint8_t line)
{
    for (uint8_t r = 0; r < CJK_GLYPH_H; r++) {
        uint16_t pix   = ((uint16_t)bmp[r * 2u] << 8) | bmp[r * 2u + 1u];
        uint8_t fb_row = line + (uint8_t)(r >> 3u);  /* line + r/8 */
        uint8_t bit    = (uint8_t)(r & 7u);           /* LSB = top  */
        if (fb_row >= 8u) break;
        for (uint8_t c = 0; c < CJK_GLYPH_W; c++) {
            if (pix & (uint16_t)(0x8000u >> c)) {
                uint8_t col = x + c;
                if (col < 128u)
                    gFrameBuffer[fb_row][col] |= (uint8_t)(1u << bit);
            }
        }
    }
}

void UI_PrintStringUTF8(const char *pString, uint8_t x, uint8_t line)
{
    const uint8_t *p = (const uint8_t *)pString;
    while (*p && x < 128u) {
        uint32_t cp = s_utf8_next(&p);
        if (cp == 0u) break;
        if (cp == (uint32_t)' ') {
            x += 4u;
        } else if (cp > (uint32_t)' ' && cp < 127u) {
            /* Printable ASCII — use gFontBig (7px wide, 14px tall) */
            unsigned int idx = (unsigned int)cp - (unsigned int)' ' - 1u;
            if ((unsigned int)x + 7u <= 128u) {
                memcpy(gFrameBuffer[line    ] + x, &gFontBig[idx][0], 7u);
                if (line + 1u < 8u)
                    memcpy(gFrameBuffer[line + 1u] + x, &gFontBig[idx][7], 7u);
            }
            x += 8u;  /* 7px + 1px gap */
        } else if (cp >= 0x0100u) {
            /* Multibyte — look up in CJK glyph cache */
            uint8_t bmp[CJK_GLYPH_BYTES];
            if (CJK_FontAvailable() && CJK_GetGlyph((uint16_t)cp, bmp))
                s_draw_cjk(bmp, x, line);
            x += (uint8_t)(CJK_GLYPH_W + 1u);  /* 12px + 1px gap */
        }
    }
}
