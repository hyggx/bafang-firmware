...
void SETTINGS_InitEEPROM(void)
{
    ...
    // Detects structural EEPROM changes independent of firmware version string.
    // If the stored byte is 0xFF (erased) or a lower version, write the current
    // version.  When a future version requires data migration, add cases here.
    //
    {
        uint8_t storedLayout = 0xFFu;
        PY25Q16_ReadBuffer(EEPROM_LAYOUT_VERSION_ADDR, &storedLayout, 1);

        if (storedLayout != EEPROM_LAYOUT_VERSION)
        {
    ...
    //
    // Version check
    // Read stored version from EEPROM and compare with VERSION_STRING
    // 
    {
        ...
        if (strncmp(storedVersion, VERSION_STRING, sizeof(storedVersion)) != 0)
        {
            // Different version: new install or firmware update

            // 1. Write new version to EEPROM
            char newVersion[16] = {0};
            strncpy(newVersion, VERSION_STRING, sizeof(newVersion));
...
gEeprom.FreqChannel[1]   = IS_FREQ_CHANNEL(Data16[5]) ? Data16[5] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
    ...
    */
    ...
    {
        uint8_t lang_byte = 0xFFu;
        PY25Q16_ReadBuffer(EEPROM_LANGUAGE_ADDR, &lang_byte, 1);
        gEeprom.LANGUAGE = (lang_byte < LANG_COUNT) ? (LangId_t)lang_byte : LANG_EN;
        g_lang = gEeprom.LANGUAGE;
    }
}

void SETTINGS_SaveLanguage(LangId_t lang)
{
    if (lang >= LANG_COUNT)
        lang = LANG_EN;
    gEeprom.LANGUAGE = lang;
    g_lang           = lang;
    uint8_t b = (uint8_t)lang;
    PY25Q16_WriteBuffer(EEPROM_LANGUAGE_ADDR, &b, 1, false);
}
...
void SETTINGS_FetchChannelName(char *s, const uint16_t channel)
{
    ...
    int i;
    for (i = 0; i < (int)(CHANNEL_NAME_MAX_BYTES - 1); i++)
        if ((uint8_t)s[i] < 0x20u || (uint8_t)s[i] == 0xFFu)
            break;

    s[i--] = '\0';                // null term — at most s[14], always in-bounds
...
void SETTINGS_SaveChannelName(uint16_t channel, const char * name)
{
    ...
    // 0x0F50
    PY25Q16_WriteBuffer(0x004000 + offset, buf, CHANNEL_NAME_MAX_BYTES, false);
}
